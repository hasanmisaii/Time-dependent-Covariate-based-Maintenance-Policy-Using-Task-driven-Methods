...
Third Scenario: there are three perfect corrective maintenance actions with different costs
while I_1 and I_2 are Random with masked data
"""
import numpy as np
import pandas as pd
    ...
    for j in range(J):
        ...
        for i in range(k):
            ...
            ex_cost[j] += ((mm) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_pcm) *\
                ...
                ((mm) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_icm) *\
                quad(integrand_1, (i)*tau, (i+1)*tau, args=(a, b))[0] * R * (quad(DL_1, 0, (i+1)*tau, args=(a_L1, b_L1))[0] * quad(DL_2, (i+1)*tau, math.inf, args=(a_L2, b_L2))[0]) +\
                    ((mm) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_mcm) *\
                quad(integrand_1, (i)*tau, (i+1)*tau, args=(a, b))[0] * R * (quad(DL_1, (i+1)*tau, math.inf, args=(a_L1, b_L1))[0])
        ...
        for i in range(k):
            ...
            if midpoint <= tau:
                ex_cost[j] += ((mm) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_pcm) *\
                    ...
                    ((mm) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_icm) *\
                    cycle_proba * (quad(DL_1, 0, (i+1)*tau, args=(a_L1, b_L1))[0] * quad(DL_2, (i+1)*tau, math.inf, args=(a_L2, b_L2))[0]) +\
                    ((mm) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_mcm) *\
                    cycle_proba * (quad(DL_1, (i+1)*tau, math.inf, args=(a_L1, b_L1))[0])
...
result_estimate_PRF = long_run_cost_estimate_PRF_v(K, tau_vals, Partition)
result_estimate_PRF
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_NPRF_v(K, 0.2, Partition)

result_estimate_NPRF = long_run_cost_estimate_NPRF_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_PKNN_v(K, 0.2, Partition)

result_estimate_PKNN = long_run_cost_estimate_PKNN_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_NPKNN_v(K, 0.2, Partition)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPKNN = long_run_cost_estimate_NPKNN_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_PGNB_v(K, 0.2, Partition)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PGNB = long_run_cost_estimate_PGNB_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_NPGNB_v(K, 0.2, Partition)
# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPGNB = long_run_cost_estimate_NPGNB_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_PSVM_v(K, 0.2, Partition)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PSVM = long_run_cost_estimate_PSVM_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_NPSVM_v(K, 0.2, Partition)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPSVM = long_run_cost_estimate_NPSVM_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_PLDA_v(K, 0.2, Partition)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PLDA = long_run_cost_estimate_PLDA_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_NPLDA_v(K, 0.2, Partition)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPLDA = long_run_cost_estimate_NPLDA_v(K, tau_vals, Partition)
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_PDT_v(K, 0.2, Partition)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PDT = long_run_cost_estimate_PDT_v(K, tau_vals, Partition)
result_estimate_PDT
...
long_run_cost_real_v(K, 0.2)
long_run_cost_estimate_NPDT_v(K, 0.2, Partition)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPDT = long_run_cost_estimate_NPDT_v(K, tau_vals, Partition)
