...
def long_run_cost_real(k, tau):
    ...
    for j in range(J):
        ...
        for i in range(k):
            ...
            # print(midpoint)
            # if midpoint <= tau:
            ex_cost[j] += ((i+1) * cost_ins/J + ex_cost_0 + ((i+1) * tau - midpoint) * cost_unavail + cost_pcm) *\
...
# report performance
print('Accuracy NB with CV: %.3f (%.3f)' % (mean(n_scores), std(n_scores)))
# make predictions using random forest for classification
# fit the model on the whole dataset
...
plt.plot(tau_vals, result_estimate_NPLDA, linestyle=(5, (5, 1)), color="m")
plt.plot(tau_vals, result_estimate_NPDT, linestyle=(6, (6, 1)), color="c")
plt.legend(["Real", "NPRF", "NPSVM", "NPKNN", "NPNB", "NPLDA"], loc="upper right",
           bbox_to_anchor=(1, 1), ncol=3, fancybox=True, shadow=True)
plt.ylabel('Rate of cost')
plt.xlabel('\u03C4')
