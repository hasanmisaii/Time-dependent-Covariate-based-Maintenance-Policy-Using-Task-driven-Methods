...
- The repairs in each period are perfect corrective maintenance with different costs
- The borders between periods are fixed. 
- First Scenario: there are three perfect corrective maintenance actions with different costs
while I_1 and I_2 are fixed
"""
import numpy as np
import pandas as pd
...
def long_run_cost_real(k, tau, I_1, I_2):
    ...
    for j in range(J):
        ...
        for i in range(k):
            ...
            ex_cost[j] += ((i+1) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_pcm) *\
                quad(integrand_1, (i)*tau, (i+1)*tau, args=(a, b))[0] * R * (((i+1) * tau) >= I_2) +\
                ((i+1) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_icm) *\
                quad(integrand_1, (i)*tau, (i+1)*tau, args=(a, b))[0] * R * (I_1 <= ((i+1) * tau) < I_2) +\
                    ((i+1) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_mcm) *\
                quad(integrand_1, (i)*tau, (i+1)*tau, args=(a, b))[0] * R * (((i+1) * tau) < I_1)
...
long_run_cost_real_v(10, 0.2, 0.5, 1)
...
b_L1 = np.mean(lam)
b_L2 = np.mean(lam)
I1 = weibull_min.rvs(c=a_L1, loc=0, scale=b_L1, size=1)
I2 = 0
while I2 < I1:
    I2 = weibull_min.rvs(c=a_L2, loc=0, scale=b_L2, size=1)/np.exp(-pow(I1/b_L2, a_L2))
...
result_real = long_run_cost_real_v(K, tau_vals, I1, I2)
result_real
...
def long_run_cost_estimate_PRF(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    a, b = Estimate_Par(t)
    for j in range(J):
        ...
        for i in range(k):
            ...
            if midpoint <= tau:
                ex_cost[j] += ((i+1) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_pcm) *\
                    cycle_proba * (((i+1) * tau) >= I_2) +\
                    ((i+1) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_icm) *\
                    cycle_proba * (I_1 <= ((i+1) * tau) < I_2) +\
                    ((i+1) * cost_ins/J + ((i+1) * tau - midpoint) * cost_unavail + cost_mcm) *\
                    cycle_proba * (((i+1) * tau) < I_1)
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_PRF_v(K, 0.2, 100, I1, I2)

result_estimate_PRF = long_run_cost_estimate_PRF_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_NPRF(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    for j in range(J):
        if j == 0:
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_NPRF_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPRF = long_run_cost_estimate_NPRF_v(K, tau_vals, Partition, I1, I2)
...
plt.plot(tau_vals, result_estimate_PRF, color='black')
plt.plot(tau_vals, result_estimate_NPRF, color='red')
plt.plot(tau_vals, result_real, color='blue')
plt.ylabel('OPTIMAL')
plt.show()
# %% KNN
...
def long_run_cost_estimate_PKNN(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    a, b = Estimate_Par(t)
    for j in range(J):
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_PKNN_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PKNN = long_run_cost_estimate_PKNN_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_NPKNN(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    for j in range(J):
        if j == 0:
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_NPKNN_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPKNN = long_run_cost_estimate_NPKNN_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_PGNB(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    a, b = Estimate_Par(t)
    for j in range(J):
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_PGNB_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PGNB = long_run_cost_estimate_PGNB_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_NPGNB(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    for j in range(J):
        if j == 0:
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_NPGNB_v(K, 0.2, Partition, I1, I2)
# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPGNB = long_run_cost_estimate_NPGNB_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_PSVM(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    a, b = Estimate_Par(t)
    for j in range(J):
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_PSVM_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PSVM = long_run_cost_estimate_PSVM_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_NPSVM(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    for j in range(J):
        if j == 0:
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_NPSVM_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPSVM = long_run_cost_estimate_NPSVM_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_PLDA(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    a, b = Estimate_Par(t)
    for j in range(J):
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_PLDA_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PLDA = long_run_cost_estimate_PLDA_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_NPLDA(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    for j in range(J):
        if j == 0:
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_NPLDA_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPLDA = long_run_cost_estimate_NPLDA_v(K, tau_vals, Partition, I1, I2)
...
def long_run_cost_estimate_PDT(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    a, b = Estimate_Par(t)
    for j in range(J):
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_PDT_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_PDT = long_run_cost_estimate_PDT_v(K, tau_vals, Partition, I1, I2)
result_estimate_PDT
...
def long_run_cost_estimate_NPDT(k, tau, partition, I_1, I_2):
    ex_cost = list(ite.repeat(0, J))
    ex_time = list(ite.repeat(0, J))
    for j in range(J):
        if j == 0:
...
long_run_cost_real_v(K, 0.2, I1, I2)
long_run_cost_estimate_NPDT_v(K, 0.2, Partition, I1, I2)

# tau_vals = np.linspace(10, 200, 100)
result_estimate_NPDT = long_run_cost_estimate_NPDT_v(K, tau_vals, Partition, I1, I2)
